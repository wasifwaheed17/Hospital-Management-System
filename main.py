...
def signup():
    if request.method=="POST":
        ...
        dobi=request.form.get('dob')
        # print(srfid,email,dob)
        encpassword=generate_password_hash(str(dobi))
...
def delete_booking(booking_id):
    try:
        # Try converting booking_id to an integer
        booking_id = int(booking_id)
        booking = Bookingpatient.query.filter_by(srfid=booking_id).first()

        if booking:
            bedtype = booking.bedtype
            hospital_code = booking.hcode

            # Increase the available bed count based on the bed type
            hospital_data = Hospitaldata.query.filter_by(hcode=hospital_code).first()
            if bedtype == "NormalBed":
